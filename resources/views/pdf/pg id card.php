<?php
    ...
    // Create custom-sized PDF exactly matching ID-1 (credit-card) dimensions ~ 86 × 54 mm in landscape
    ...
    foreach ($data as $row) {
        ...
        // Registrar signature
        $pdf -> Image('card/registrar sign.png',44,35,20,5,'PNG');

        // Issue and expiry dates
        $pdf->SetY(44.3);
        $pdf->SetX(35);
        $pdf->SetFont('Arial', 'B', 6);
        $pdf->Cell(86, 5, $row->issue_date ? date('d/m/Y', strtotime($row->issue_date)) : 'N/A', 0, 1, 'L');

        $pdf->SetY(48.7);
        $pdf->SetX(35);
        $pdf->SetFont('Arial', 'B', 6);
        $pdf->Cell(86, 5, $row->expire_date ? date('d/m/Y', strtotime($row->expire_date)) : 'N/A', 0, 1, 'L');
