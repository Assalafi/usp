...
$users = DB::table('hostel')->where(['id' => $id])->get();
 ...
 foreach ($users as $key => $row){
    ...
    $names = Student::where(['username' => $row -> occupant])->first();
    $name = $names->first_name .' '. $names->last_name .' '. $names->other_name;
     $dept = $names->programs->title;
     ...
     $table->easyCell('','img:uploads/sign2.png, w40; colspan:11; font-size:12; align:L;border:0; font-style:U;');
     $table->printRow();
     $table->easyCell('Prof. Ahmadu M. Brono', 'colspan:11; font-size:12; align:L;border:0; font-style:B;');
     $table->printRow();
