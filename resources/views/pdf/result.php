...
foreach ($dept as $dp) {
    ...
    $columnWidths = [];
    $heading = 0;
    $grading_type = DB::table('results')->where(['code' => $course, 'session' => $session, 'department' => $dp->department])->groupBy('grading')->select('grading')->get('grading');
    foreach ($grading_type as $gt) {
        ...
        $data = DB::table('results')->where(['code' => $course, 'session' => $session, 'department' => $dp->department, 'grading' => $gt->grading])->orderBy('username', 'ASC')->get();
        if ($heading == 1) {
            // Table header
...
// use i with result code and session
$pdf->Output('D', $name . '.pdf', true);
// $pdf->Output();
die;
