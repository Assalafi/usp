...
// Custom title case function that handles slashes and small words
function properTitleCase($string) {
    // Small words that should be lowercase unless they're the first word
    $smallWords = ['and', 'or', 'the', 'a', 'an', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'from', 'other', 'etc'];
...
foreach ($certificates as $cert) {
    ...
    $pdf->AddFont('Times', '', 'times.php');                         // 6. Times New Roman
    ...
    if ($showBg == '1') {
        $bgPath = public_path('uploads/cert-type3.png');
    ...
    // --- ID Number (on dotted line, right of logo) --- 70.6
    $pdf->SetFont('Calligrapher', '', 17);
    ...
    $nameSize = 25;            // <-- CHANGE THIS to adjust size
    $pdf->SetFont($nameFont, '', $nameSize);
    $studentName = ucwords(strtolower($cert['student_name']));
    $pdf->SetXY(30, 134.5);
    $pdf->Cell(150, 12, $studentName, 0, 0, 'C');

    // --- Degree (on dotted line after body text, centered)... ---AAA
    $degree = $cert['degree'];
    $fontSize = strlen($degree) > 45 ? 18 : 22;
    ...
    //$pdf->SetFont('Calligrapher', '', 22);
    $department = properTitleCase($cert['department']);
    $fontSize = strlen($department) > 45 ? 18 : 22;
    $pdf->SetFont('Calligrapher', '', $fontSize);
    $pdf->SetXY(30, 226);
    ...
    // Position day on "Given this.......day of......20....._____." aaa
    ...
    $pdf->Image($qrTempFile, 30, 33, $qrSize, $qrSize);
