...
$getCourses = DB::table('results')->distinct('code')->pluck('code');
$faculties = DB::table('course')->whereIn('code', $getCourses)->distinct('faculty')->get('faculty');
foreach ($faculties as $row) {
    $program = DB::table('course')->whereIn('code', $getCourses)->where('faculty', $row->faculty)->distinct('program')->get('program');
    foreach ($program as $rowp){
        $results = DB::table('results')->where(['program' => $rowp->program])->select('code')->groupBy('code')->get();
        foreach ($results as $rowr){
            echo ''. $row->faculty .' '.$rowp->program .' '.$rowr->code .'<br>';
        }
    }
...
foreach($dept as $dp){
 ...
 $table->printRow();
 $table->easyCell('Course Code/Title    '.ucwords(strtolower($course)).': '.ucwords(strtolower($title)).'  Units: '.$unit, 'colspan:14; font-size:12; align:C;border:0;');
 $table->printRow();
...
$table->easyCell("Total", 'colspan:5');
 ...
 if(($a+$b+$c+$d+$e+$f)<10){$table->easyCell('0'.$a+$b+$c+$d+$e+$f, 'colspan:3');}else{$table->easyCell($a+$b+$c+$d+$e+$f, 'colspan:3');}
 $table->printRow();
