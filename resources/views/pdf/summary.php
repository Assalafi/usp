...
$getCourses = DB::table('results')->whereIn('approve', ['dean', 'vc'])->distinct('code')->pluck('code');
...
foreach ($faculties as $row) {
    ...
    $p = DB::table('results')->whereIn('program', $pro)->whereIn('approve', ['cs'])->where(['session' => session('system_session')])->exists();
    //if ($p) {
    ...
    foreach ($programs as $rowp) {
        ...
        $results = DB::table('results')->whereIn('approve', ['dean', 'dean'])->where(['program' => $rowp->program])->select('code')->groupBy('code')->get();
        $code = '';
