...
var nigeriaLGAs = {
    ...
    "Katsina":["Batagarawa","Batsari","Baure","Bindawa","Charanchi","Dandume","Danja","Dan-Musa","Daura","Dutsi-Ma","Dutsin-Ma","Faskari","Funtua","Ingawa","Jibia","Junto","Kafur","Kaita","Kankara","Kankia","Katsina","Kurfi","Kusada","Mai'Adua","Malumfashi","Mani","Mashi","Matazu","Musawa","Rimi","Runka","Safana","Sandamu","Zango"],
    "Kebbi":["Argungu","Arewa Dandi","Augie","Bagudo","Birnin Kebbi","Bunza","Dandi","Danko-Wasagu","Fakai","Gwandu","Gyedu","Jega","Kalgo","Kamba","Kebbi","Koko/Besse","Maiyama","Ngaski","Sakaba","Shanga","Suru","Wasagu/Danko","Yauri","Zuru"],
    "Kogi":["Adavi","Ajaokuta","Ankpa","Bassa","Dekina","Ibaji","Idah","Igalamela-Odolu","Ijumu","Kabba/Bunu","Kogi","Koton-Karfe","Lokoja","Mopa-Muro","Ofu","Ogori/Magongo","Okehi","Okene","Olamaboro","Omala","Oyi","Panda","Yagba East","Yagba West"],
...
// Function to initialize state/LGA for edit mode
function initStateLGAEdit(stateSelect, lgaSelect, currentState, currentLga) {
    if (currentState) {
        // Find the matching key in nigeriaLGAs (case-insensitive)
        var stateKeyToSet = '';
        for (var key in nigeriaLGAs) {
            if (key.toUpperCase() === currentState.toUpperCase()) {
                stateKeyToSet = key;
                break;
            }
        }
        ...
        if (stateKeyToSet) {
            // Set the dropdown value to the exact key from nigeriaLGAs
            $(stateSelect).val(stateKeyToSet);

            // Manually load LGAs
            var $lga = $(lgaSelect);
            if (nigeriaLGAs[stateKeyToSet]) {
                var html = '<option value="">Select LGA</option>';
                nigeriaLGAs[stateKeyToSet].forEach(function(l) { html += '<option value="'+l+'">'+l+'</option>'; });
                $lga.html(html).prop('disabled', false);
            }

            // Set LGA value immediately after loading
            if (currentLga) {
                var found = false;
                $lga.find('option').each(function() {
                    if ($(this).val().toUpperCase() === currentLga.toUpperCase()) {
                ...
                if (!found) {
                    $lga.find('option').each(function() {
                        if ($(this).text().toUpperCase() === currentLga.toUpperCase()) {
                            $(this).prop('selected', true);
                            return false;
                        }
                    });
