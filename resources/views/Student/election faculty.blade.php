                                    ...
                                    <div class="card-block">
                                        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                                            @php
                                                $active = 1;
                                            @endphp
                                            @foreach ($poss->where('category', $category)->where('position', $level) as $pos)
                                                <li class="nav-item">
                                            ...
                                            @foreach ($poss->where('category', $category)->where('position', $level) as $pos)
                                                <div class="tab-pane fade @if ($active == 1) show active @endif"
                                                    ...
                                                    <div class="card-deck row">
                                                        @foreach ($data->where('category', $category)->where('position', $pos->position)->where('faculty', $faculty)->where('level', $level) as $row)
                                    ...
                                    <table id=""
                                        ...
                                        <tbody>
                                            @php
                                                $sn = 1;
                                            @endphp
                                            @foreach ($data->where('category', $category)->where('faculty', $faculty)->where('level', $level) as $row)
                                                <tr>
                                                    <td>{{ $sn++ }}</td>
                                                    <td>{{ $row->candidate }}</td>
                                                    <td>{{ $row->name }}</td>
