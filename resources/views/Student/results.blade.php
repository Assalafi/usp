...
<div class="main-body ug-results-page">
    <div class="page-wrapper">
        <div class="ug-results-shell">
            ...
            <section class="ug-results-summary" aria-label="Results summary">
                <div class="ug-result-stat"><span class="ug-result-stat-icon blue"><i class="fas fa-book"></i></span><div><strong>{{ $courseCount }}</strong><small>Approved courses</small></div></div>
                <div class="ug-result-stat"><span class="ug-result-stat-icon cyan"><i class="fas fa-layer-group"></i></span><div><strong>{{ $unitCount }}</strong><small>Total units</small></div></div>
                <div class="ug-result-stat"><span class="ug-result-stat-icon green"><i class="fas fa-circle-check"></i></span><div><strong>{{ $passedCount }}</strong><small>Passed</small></div></div>
                <div class="ug-result-stat"><span class="ug-result-stat-icon red"><i class="fas fa-circle-exclamation"></i></span><div><strong>{{ $failedCount }}</strong><small>Carryover</small></div></div>
            </section>

            <section class="ug-results-context">
                <div>
                    <span class="ug-results-kicker">{{ $displaySession }}</span>
                    <h2>{{ $studentName }}</h2>
            ...
            @if ($studentResults->isNotEmpty())
                <div class="ug-results-toolbar">
                    <div><strong>Course results</strong><span>Search by course code or title</span></div>
                                        ...
                                        <div class="ug-result-cards">
                                            @foreach ($semesterResults as $result)
                                                ...
                                                <article class="ug-result-card" data-result-card data-search-text="{{ $searchText }}">
                                                    ...
                                                    <div class="ug-result-meta"><span><i class="fas fa-cubes"></i> {{ $result->unit ?? '—' }} unit{{ (float) ($result->unit ?? 0) === 1.0 ? '' : 's' }}</span><span><i class="fas fa-calendar"></i> {{ $semester ?: 'Semester not specified' }}</span><span class="ug-approved"><i class="fas fa-shield-check"></i> Approved</span></div>
                                                </article>
                                            @endforeach
                                        </div>
                                    </div>
